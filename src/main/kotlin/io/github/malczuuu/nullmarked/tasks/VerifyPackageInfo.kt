 ...
 * not excluded by [packageSelectionRules]. When [verificationEnabled] is `false`, the task passes without checking
 * anything.
 *
 ...
 */
@CacheableTask
abstract class VerifyPackageInfo : DefaultTask() {
  ...
  /**
   * Ordered package rules deciding which packages are verified, in the same encoded form and for the same reasons as
   * [GeneratePackageInfo.packageSelectionRules]. Configure them through the `nullmarked { packages { ... } }` block,
   * see [NullMarkedPackagesSpec].
   */
