...
class GeneratePackageInfoCommonsTest {
  ...
  @Test
  fun `prunes files for packages no longer expected`() {
    val outputDir = testProject.file("build/generated-package-info")
    val stale =
        TestProject.packageInfoUnder(outputDir, "com.acme").apply {
          parentFile.mkdirs()
    ...
    prunePackages(mapOf("com.acme" to stale), emptyMap(), outputDir)

    assertThat(stale).doesNotExist()
  }

  @Test
  fun `keeps files for packages still expected`() {
    val outputDir = testProject.file("build/generated-package-info")
    val kept =
        TestProject.packageInfoUnder(outputDir, "com.acme").apply {
          parentFile.mkdirs()
    ...
    prunePackages(mapOf("com.acme" to kept), mapOf("com.acme" to "package com.acme;"), outputDir)

    assertThat(kept).exists()
  }

  @Test
  fun `prunes now-empty parent directories up to the output directory`() {
    val outputDir = testProject.file("build/generated-package-info")
    val stale =
        TestProject.packageInfoUnder(outputDir, "com.acme.util").apply {
          parentFile.mkdirs()
          writeText("package com.acme.util;")
        }

    prunePackages(mapOf("com.acme.util" to stale), emptyMap(), outputDir)

    assertThat(outputDir.resolve("com/acme/util")).doesNotExist()
    assertThat(outputDir.resolve("com/acme")).doesNotExist()
    assertThat(outputDir).exists()
  }

  @Test
  fun `writes a new package-info when none exists yet`() {
    val outputDir = testProject.file("build/generated-package-info").apply { mkdirs() }

    ensurePackageInfoFile(emptyMap(), "com.acme", outputDir, "package com.acme;\n")

    assertThat(TestProject.packageInfoUnder(outputDir, "com.acme")).hasContent("package com.acme;\n")
  }

  @Test
  fun `rewrites an existing package-info when its content differs`() {
    val outputDir = testProject.file("build/generated-package-info")
    val existing =
        TestProject.packageInfoUnder(outputDir, "com.acme").apply {
          parentFile.mkdirs()
    ...
    ensurePackageInfoFile(mapOf("com.acme" to existing), "com.acme", outputDir, "package com.acme;\n")
  ...
  fun `leaves an up-to-date package-info untouched`() {
    ...
    val lastModifiedBefore = existing.lastModified()
